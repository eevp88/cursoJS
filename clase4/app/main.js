...
const loadRegisterTemplate = ()=>{
	const template = `
		...
		<a href="#"  id="register">Iniciar Sesión</a>
		<div id="error"></div>
	`
	const body = document.getElementsByTagName('body')[0]
	body.innerHTML = template
...
const gotoListener = action => {
	const gotoAction = document.getElementById(`${action}`)
	gotoAction.onclick = (e) =>{
		e.preventDefault()
		(action == 'register')? registerPege() : loginPage()
	}
}
